...
describe('Test in <Avatar />', () => {
  test('must be render component <Avatar />', () => {
    const wrapper = shallow(<Avatar size={55} />)
    expect(wrapper).toMatchSnapshot()
  })

  test('component <Avatar /> must be image src', () => {
    const imgUrl =
    ...
    const wrapper = shallow(<Avatar size={55} urlAvatar={imgUrl} />)
    const validateImage = wrapper.find('img').prop('src')
    expect(imgUrl).toEqual(validateImage)
  })

  test('component <Avatar /> must be image src are default', () => {
    const imgUrl =
